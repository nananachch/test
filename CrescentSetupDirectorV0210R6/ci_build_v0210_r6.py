...
def main() -> None:
    ...
    source_archive = HERE / 'source_v0.2.10.r6.tar.bz2'
    if not source_archive.exists():
        raise SystemExit('R6 source archive missing')
    actual_archive_sha = sha(source_archive)
    print(f'R6 source archive SHA-256: {actual_archive_sha}')
    try:
        with tarfile.open(source_archive, 'r:bz2') as tf:
    ...
    proof={
        ...
        'source_archive_sha256':actual_archive_sha,'source_zip_sha256':sha(source_zip),
