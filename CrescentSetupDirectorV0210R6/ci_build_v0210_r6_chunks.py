...
WORK = Path(os.environ.get('RUNNER_TEMP', HERE / '_tmp')) / 'csd_v0210_r6_chunks'
SOURCE = WORK / 'source'
...
def main() -> None:
    ...
    chunks = sorted((HERE / 'chunks3').glob('source_v0.2.10.r6.tbz2.b64.*'))
    if len(chunks) != 5:
        raise SystemExit(f'expected 5 R6 chunks, found {len(chunks)}')
    b64 = ''.join(p.read_text(encoding='ascii').strip() for p in chunks)
    archive = WORK / 'source_v0.2.10.r6.tar.bz2'
    archive.write_bytes(base64.b64decode(b64, validate=True))
    archive_sha = sha(archive)
    print(f'R6 reconstructed source archive SHA-256: {archive_sha}')
    try:
        with tarfile.open(archive, 'r:bz2') as tf:
            tf.extractall(SOURCE, filter='data')
    except Exception as ex:
        raise SystemExit(f'R6 source archive extraction failed: {ex}')

    setup_text=(SOURCE/'SetupEngine.cs').read_text(encoding='utf-8')
    required = ['FailOpenCriticalEncounter','FailOpenMagicPot','[POT-FAIL-OPEN]','RunCeBurstPriority','RunBlackRegimentOracle']
    for item in required:
        if item not in setup_text:
    ...
    proof={
        ...
        'source_archive_sha256':archive_sha,'source_zip_sha256':sha(source_zip),
